...
(function () {
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, imageUploadHandler) {
        ...
        function makeSpritedButtonRow() {
            ...
            makeSpacer(1);
            buttons.link = makeButton("wmd-link-button", gettext("Hyperlink (Ctrl+L)"), "-80px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, false);
            }));
            buttons.quote = makeButton("wmd-quote-button", gettext("Blockquote (Ctrl+Q)"), "-100px", bindCommand("doBlockquote"));
            buttons.code = makeButton("wmd-code-button", gettext("Code Sample (Ctrl+K)"), "-120px", bindCommand("doCode"));
            buttons.image = makeButton("wmd-image-button", gettext("Image (Ctrl+G)"), "-140px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, true, imageUploadHandler);
            }));
            makeSpacer(2);
            buttons.olist = makeButton("wmd-olist-button", gettext("Numbered List (Ctrl+O)"), "-160px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.ulist = makeButton("wmd-ulist-button", gettext("Bulleted List (Ctrl+U)"), "-180px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.heading = makeButton("wmd-heading-button", gettext("Heading (Ctrl+H)"), "-200px", bindCommand("doHeading"));
            buttons.hr = makeButton("wmd-hr-button", gettext("Horizontal Rule (Ctrl+R)"), "-220px", bindCommand("doHorizontalRule"));
            makeSpacer(3);
            buttons.undo = makeButton("wmd-undo-button", gettext("Undo (Ctrl+Z)"), "-240px", null);
    ...
    commandProto.doYellowHighlight = function (chunk, postProcessing) {
        return this.doYorB(chunk, postProcessing, 1, "yellow highlighted text");
    };

    commandProto.doBlueHighlight = function (chunk, postProcessing) {
        return this.doYorB(chunk, postProcessing, 2, "blue highlighted text");
    };
